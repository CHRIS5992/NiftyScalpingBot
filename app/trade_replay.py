...
def get_day_data(
    ...
    day_trades = tdf[tdf['entry_date'] == selected_date].copy() if not tdf.empty else pd.DataFrame()

    if day_bars.empty:
        return {'day_bars': day_bars, 'day_trades': day_trades, 'summary': {}}

    summary = {
        'date': selected_date,
        'spot_open': round(day_bars['close'].iloc[0], 2),
        'spot_close': round(day_bars['close'].iloc[-1], 2),
        'spot_high': round(day_bars['close'].max(), 2),
        'spot_low': round(day_bars['close'].min(), 2),
        'day_range': round(day_bars['close'].max() - day_bars['close'].min(), 2),
...
def get_available_dates(tdf: pd.DataFrame) -> list[str]:
    ...
    if tdf.empty:
        return []
    return sorted(tdf['entry_date'].unique().tolist())
